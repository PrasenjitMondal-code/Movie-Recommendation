...
def load_data(file_path, labeled=True):
    sentences = []
    labels = []
...
train_sentences, train_labels = load_data('data/train.txt')
dev_sentences, dev_labels = load_data('data/dev.txt')
test_sentences = load_data('data/test-blind.txt', labeled=False)

# Print the first few samples to understand the data
print("Training data sample:", train_sentences[:3], train_labels[:3])
print("Development data sample:", dev_sentences[:3], dev_labels[:3])
print("Test data sample:", test_sentences[:3])
